...
group by k.ho_ten;	
-- câu 9
select year(h.ngay_lam_hop_dong), month(h.ngay_lam_hop_dong) as thang, count(h.ma_khach_hang) as so_luong_khach_hang
from hop_dong h
...
select h.ma_hop_dong, h.ngay_lam_hop_dong, h.ngay_ket_thuc_hop_dong, h.tien_dat_coc, ifnull(sum(hc.so_luong),0) as so_luong_dich_vu_kem_theo
from hop_dong h
left join hop_dong_chi_tiet hc on hc.ma_hop_dong = h.ma_hop_dong
left join dich_vu_di_kem d on d.ma_dich_vu_di_kem = hc.ma_dich_vu_di_kem
group by h.ma_hop_dong
;

-- Câu 11
select h.ma_hop_dong, d.ten_dich_vu_di_kem
from hop_dong h
left join hop_dong_chi_tiet hc on hc.ma_hop_dong = h.ma_hop_dong
left join dich_vu_di_kem d on d.ma_dich_vu_di_kem = hc.ma_dich_vu_di_kem
left join khach_hang k on k.ma_khach_hang = h.ma_khach_hang
left join loai_khach l on l.ma_loai_khach = k.ma_loai_khach
group by d.ten_dich_vu_di_kem
having (l.ten_loai_khach = "Diamond" and ( k.dia_chi like "%Vinh%" or k.dia_chi like "%Quảng Ngãi%"))
;
